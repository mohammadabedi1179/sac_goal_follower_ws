...
# Add a global stereo matcher (run once)
stereo = cv2.StereoSGBM_create(
    minDisparity=0,
    numDisparities=128,      # 128 px = max ~20 m
    blockSize=7,
    P1=8*3*7**2,
    P2=32*3*7**2,
...
class StereoBoxDepthNode(Node):
    def __init__(self):
        ...
        self.left_img = None
        self.right_img = None
        self.left_sub_img = self.create_subscription(Image, '/follower_robot/depth_cam/left/image_rect', self.left_img_cb, 10)
        self.right_sub_img = self.create_subscription(Image, '/follower_robot/depth_cam/right/image_rect', self.right_img_cb, 10)
        self._disp_map = None
        self._disp_map_timestamp = None
        ...
        self.get_logger().info('Advanced stereo_box_depth with disparity calculations')
    ...
    def try_match(self):
        ...
        for ld in self.last_left.detections:
            ...
            disparity = self._disparity_from_dense_map(lbox)  # left box only!
            if not disparity or disparity < 3.0:
                continue
    ...
    def _disparity_from_dense_map(self, lbox):
        if self.left_img is None or self.right_img is None:
            return None

        # Compute disparity map once per frame (cached)
        if not hasattr(self, '_disp_map') or self._disp_map_timestamp != self.last_left.header.stamp:
            if self.left_img.shape == self.right_img.shape:
                disp = stereo.compute(self.left_img, self.right_img).astype(np.float32) / 16.0
                disp[disp <= 0] = np.nan
                self._disp_map = disp
                self._disp_map_timestamp = self.last_left.header.stamp

        x1, y1, x2, y2 = map(int, lbox)
        # Use lower 70% of box (torso, more stable)
        y1 = int(y1 + 0.3*(y2-y1))
        roi = self._disp_map[y1:y2, x1:x2]
        if roi.size == 0:
            return None
        
        valid_disparities = roi[~np.isnan(roi)]
        if len(valid_disparities) < 50:  # need some points
            return None
